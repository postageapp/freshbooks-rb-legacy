...
class TestBase < Test::Unit::TestCase
  def test_establish_connection
    ...
    FreshBooksLegacy::Base.establish_connection("company.freshbooks.com", "auth_token")
    connection = FreshBooksLegacy::Base.connection
    assert_not_nil connection
    assert_equal "company.freshbooks.com", connection.account_url
    assert_equal "auth_token", connection.auth_token
    
    FreshBooksLegacy::Base.establish_connection("company2.freshbooks.com", "auth_token2")
    connection = FreshBooksLegacy::Base.connection
    assert_not_nil connection
  ...
  def test_new_from_xml_to_xml__round_trip
    ...
    item = FreshBooksLegacy::MyItem.new_from_xml(doc.root)
...
module FreshBooksLegacy
  ...
  class MyAddress < FreshBooksLegacy::Base
    define_schema do |s|
      s.string :street1
    end
  end
  
  class MyLine < FreshBooksLegacy::Base
    define_schema do |s|
